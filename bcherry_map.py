...
type_counter = {'벚꽃로': collections.Counter(), '벚꽃길': collections.Counter()}
...
total_ro = sum(type_counter['벚꽃로'].values())
total_gil = sum(type_counter['벚꽃길'].values())
total = sum(counter.values())
print(f'벚꽃로: {total_ro}건, 벚꽃길: {total_gil}건, 합계: {total}건')
...
sido_gdf['count_ro'] = sido_gdf['sidonm_new'].map(type_counter['벚꽃로']).fillna(0).astype(int)
sido_gdf['count_gil'] = sido_gdf['sidonm_new'].map(type_counter['벚꽃길']).fillna(0).astype(int)

all_data = sorted(
    [(row['sidonm_new'], row['count'], row['count_ro'], row['count_gil'])
...
ax_map.set_title('벚꽃로 + 벚꽃길 시도별 분포', fontproperties=fp(15), pad=12)
ax_map.axis('off')
...
for i, (sidonm, cnt, cnt_ro, cnt_gil) in enumerate(all_data):
    ...
    ax_tbl.text(col_x[3] + 0.10, y, f'{cnt_gil:,}', fontproperties=fp(9.5), va='center', ha='right', zorder=2)
...
ax_tbl.text(col_x[3] + 0.10, y_total, f'{total_gil:,}',  fontproperties=fp(10), fontweight='bold', va='center', ha='right')
